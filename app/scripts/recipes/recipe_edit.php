...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $sql = "UPDATE recipes SET title=?, category_id=?, ingredients=?, tools=?, prep_time=?, rest_time=?, cook_time=?, servings=?, description=?, illustration=?, user_id=? WHERE id=?";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("sissiiiissii", $title, $category_id, $ingredients, $tools, $prep_time, $rest_time, $cook_time, $servings, $description, $illustration, $author, $recipeId);
        $stmt->execute();
